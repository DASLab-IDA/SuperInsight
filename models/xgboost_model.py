...
def train(X_train: list, y_train: list):
    if os.path.exists("all label trained models/xgboost_model_corpus.m"):
        model = joblib.load('all label trained models/xgboost_model_corpus.m')
    else:
        model = xgb.XGBRegressor(colsample_bytree=0.38, gamma=0.05,
                                   learning_rate=0.05, max_depth=3,
                                   min_child_weight=1.98, n_estimators=1000,
                                   reg_alpha=0.46, reg_lambda=0.85,
                                   subsample=0.53, silent=1,
                                   random_state=2022, nthread=-1)
        model.fit(X_train, y_train)
        joblib.dump(model, 'all label trained models/xgboost_model_corpus.m')
    return model


def load_model():
    if os.path.exists("all label trained models/xgboost_model_corpus.m"):
        estimator = joblib.load('all label trained models/xgboost_model_corpus.m')
    else:
        estimator = None
    return estimator
