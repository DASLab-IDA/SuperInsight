...
def train(X_train: list, y_train: list):
    if os.path.exists("all label trained models/catboost_model_corpus.m"):
        model = joblib.load('all label trained models/catboost_model_corpus.m')
    else:
        model = CatBoostRegressor(iterations=1500, learning_rate=0.08, depth=7)
        model.fit(X_train, y_train)
        joblib.dump(model, 'all label trained models/catboost_model_corpus.m')
    return model
...
def train_with_category(X_train: list, y_train: list):
    if os.path.exists("all label trained models/catboost_model_corpus.m"):
        ...
        model = CatBoostRegressor(iterations=1500, learning_rate=0.08, depth=7, cat_features=['c1', 'c2'])
...
def load_model():
    if os.path.exists("all label trained models/catboost_model_corpus.m"):
        estimator = joblib.load('all label trained models/catboost_model_corpus.m')
    else:
        estimator = None
    return estimator
